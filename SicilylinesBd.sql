...
--
-- Hôte : 127.0.0.1
-- Généré le : jeu. 02 nov. 2023 à 15:59
...
ALTER TABLE `liaison`
  ...
  ADD KEY `I_FK_LIAISON_TRAVERSEE` (`id_traversee`);
...
ALTER TABLE `port`
  ADD PRIMARY KEY (`ID`),
  ADD KEY `I_FK_PORT_LIAISON` (`id_liaison`);
...
ALTER TABLE `liaison`
  ...
  ADD CONSTRAINT `FK_LIAISON_TRAVERSEE` FOREIGN KEY (`id_traversee`) REFERENCES `traversee` (`ID`);

--
-- Contraintes pour la table `port`
--
ALTER TABLE `port`
  ADD CONSTRAINT `FK_PORT_LIAISON` FOREIGN KEY (`id_liaison`) REFERENCES `liaison` (`ID`);
